...
def run(
    ...
    train_patterns = [
        (p.pattern_type, np.asarray(p.node_indices)) for p in alert_train + normal_train
    ]
    test_patterns = [
        (p.pattern_type, np.asarray(p.node_indices)) for p in alert_test + normal_test
    ]
    print(f"Train patterns: {len(train_patterns)}  Test patterns: {len(test_patterns)}")
    W, L, D = graph_params(G)
    # ---- target 1: baseline top-K Laplacian eigenvectors --------------------
    print("\nBuilding baseline target  R = span(U_K) ...")
    l_k, B_eig = dominant_eigenvectors(L, subspace_dim)
    ...
    with open(report_path, "w") as fh:
        ...
        header = f"{'pattern_type':>16s} | " + " | ".join(f"{n:>14s}" for n in targets)
        fh.write(header + "\n")
        fh.write("-" * len(header) + "\n")
        for t in all_types:
            row = f"{t:>16s} | " + " | ".join(
        ...
        fh.write(
            f"{'reduction':>10s} | " + " | ".join(f"{n:>14s}" for n in targets) + "\n"
        )
        for i, r in enumerate(reductions):
            fh.write(
                f"{r:10.2f} | "
                + " | ".join(f"{sweep[n][i]:14.3f}" for n in targets)
                + "\n"
            )
...
def _plot_filter(theta, out_dir):
    ...
    ax.set_title(
        r"Learned SGC filter coefficients $g_\theta(\hat A)=\sum_k\theta_k\hat A^k$"
    )
    ax.grid(alpha=0.3)
    fig.tight_layout()
