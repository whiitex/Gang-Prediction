...
from coarsening_utils_cost import coarsen
...
def get_graph(DATA):
    if DATA == 0: # create a Cora dataset
        ...
        club_labels = torch.tensor([club_to_idx[club] for club in club_labels], dtype=torch.long, device=device)
        x = torch.eye(dataset.number_of_nodes(), dtype=torch.float, device=device)
        edge_index = torch.tensor(list(dataset.edges), dtype=torch.long, device=device).t().contiguous()
...
def coarsen_and_plot(G, DATA):
    ...
    for eps in max_eps:
        ...
        plt.axhline(y=eps, color='gray', linestyle='--', linewidth=1.5, label=f"max_eps = {eps:.2f}")
        ...
        if DATA == 1: make_gif(Gall, Call, gif_path=f"{save_path}max_eps_{eps:.2f}.gif", frame_duration=100)
