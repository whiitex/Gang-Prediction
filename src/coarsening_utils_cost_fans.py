...
def coarsen(
    ...
    epsilons: list of floats
        Cumulative epsilon values at each level

    Example
    -------
    C, Gc, Call, Gall, eps_l = coarsen(G, K=10, r=0.8)
    """
    ...
    # cur_cost, max_cost_in_level, costs = 0, 0, []
    epsilon_l, max_eps_in_level, epsilons = 0, 0, [0]

    for level in range(1, max_levels + 1):

        max_eps_l += max_epsilon / max_levels
        max_sigma = (max_eps_l + 1) / (epsilon_l + 1) - 1

        iC, G, B, sigma_l = coarse_one_level(
            G,
            iC,
            B,
            K=K,
        ...
        if G.num_nodes == 1: break # coarsened to 1 node
        if G.num_nodes <= (1 - r) * N: break # reached ratio

    return C, G, Call, Gall, epsilons


def coarse_one_level_fans(
    G,
    iC,
    B,
    K=10,
    ...
    # compute coarsening_list using variation algorithms
    if "variation" in method:

        # update/init matrix A, B
        if level == 1:
            A = B
        else:
        ...
        # compute coarsening_list
        if method == "variation_edges": # variation edges
            coarsening_list, sigma_l = contract_variation_edges(
                G,
                K=K,
                A=A,
                r=r_cur,
        ...
        else: # variation neighborhoods or fans
    ...
    # compute coarsening_list using other methods
    else:
        ...
        if algorithm == "optimal":
            ...
            coarsening_list, cur_cost = matching_greedy(G, weights=weights, r=r_cur)

    # print(f'{coarsening_list=}, {len(coarsening_list)=}, {G.num_nodes=}, {G.num_edges=}')

    # construct coarsening matrix and coarsened graph
...
def contract_variation_edges(
    G: Data, A=None, K=10, r=0.5, algorithm="greedy",
    max_sigma=float('inf')
):
    """
    ...
    def subgraph_cost(A, edge, w):
        ...
        # semantic cost
        embeddings = getattr(G, "embeddings", None)
        if embeddings is None:
            embeddings = getattr(G, "x", None)
            if embeddings is not None:
                G.embeddings = embeddings

        if embeddings is not None:
            Bh = Pibot @ embeddings[edge, :]
    ...
    weights = torch.tensor(
        # [subgraph_cost(A, G.edge_index[:, e], G.edge_weight[e])[2] for e in range(M)] # both (sum)
        # [subgraph_cost(A, G.edge_index[:, e], G.edge_weight[e])[1] for e in range(M)] # only semantic cost
        [subgraph_cost(A, G.edge_index[:, e], G.edge_weight[e])[0] for e in range(M)] # only structural cost
    )
    ...
    elif algorithm == "greedy": # default
        # find a heavy weight matching; ignore returned cumulative cost here
        coarsening_list, sigma_l = matching_greedy(
            G, weights=weights, r=r,
            max_sigma=max_sigma,
        )

    return coarsening_list, sigma_l


def contract_variation_linear(
    G: Data, A, r=0.5, mode="neighborhood", candidates=None
):
    """
    ...
    def subgraph_cost(nodes):
        ...
        a = torch.sparse.mm(P, G.W)
        W_sub = torch.sparse.mm(a, P.t().coalesce())
        ...
        if embeddings is None:
            ...
            emb_sub = embeddings[nodes, :]
            P_emb_sub = emb_sub - torch.outer(ones, torch.sum(emb_sub, dim=0))

            L_P_emb = torch.sparse.mm(L, P_emb_sub) if L.is_sparse else L @ P_emb_sub
            semantic_cost = torch.norm(P_emb_sub.t() @ L_P_emb) / (nc - 1)
        
        return structural_cost, semantic_cost

    class CandidateSet:
        def __init__(self, candidate_list):
            self.set = candidate_list
            # self.cost = subgraph_cost(candidate_list)[0] # structural cost
            self.cost = subgraph_cost(candidate_list)[1] # semantic cost
    ...
    if "fans" in mode:
        assert candidates is not None, "candidates must be provided for fan-based coarsening"
        for i, fan in enumerate(candidates):
            print(i)
            family.append(CandidateSet(torch.tensor(fan)))
    ...
    while len(family) > 0 and n_reduce > 1:
...
def matching_greedy(
    ...
    * Depending on G, the algorithm might fail to return ratios>0.3
    ...
    edges = edges[:, idx] # sorted by increasing weights
    weights = weights[idx] # sorted by increasing weights
    ...
    for [i, j], cost in zip(edges.T, weights):
        i, j, cost = i.item(), j.item(), cost.item()

        # check cost threshold
        if (sigma_l_2 + cost) ** 0.5 > max_sigma: break

        # check if marked
        if any(marked[[i, j]]): continue
        
        # add it to the matching and update vars
